...
void glContextInit()
{
	...
	glContext.viewport_width = 500; //glClamp(width, (GLuint)MIN_BUFFER_WIDTH, (GLuint)MAX_BUFFER_WIDTH);
	glContext.viewport_height = 500;// glClamp(width, (GLuint)MIN_BUFFER_HEIGHT, (GLuint)MAX_BUFFER_HEIGHT);
...
void glSetError(GLenum error, const GLchar* szErrorDesc)
{
	...
	strcpy(glContext.error_desc, szErrorDesc);
}
